...
SFX_HIT_METALLIC = load_audio_safe("sword_hit_metallic.mp3") # NEW: Metallic hit sound asset load!
...
class Game:
    def __init__(self):
        ...
        self.sfx = {
            ...
            "hit_metallic": SFX_HIT_METALLIC # Registered sound asset
        }
        ...
        self.projectiles = [] 
        self.sparks = [] # NEW: Holds particle vectors for impact sparks!
    ...
    def load_hud_weapon(self, path):
        try:
            # FIXED: Removed colorkey assignments to clear black corner overlay triangles completely
    ...
    def update(self):
        ...
        # NEW: Vector update tick for hit sparks
    ...
    def draw(self):
        ...
        # --- FIXED: Generates realistic multi-color atmosphere gradients on sky canvas loops ---
        sky_h = HEIGHT // 2
        top_sky = self.get_sky_color()
        # Create a matching horizon gradient color offset
        ...
        for sprite in self.sprites:
            ...
            if px > 0.5: 
                ...
                # --- FIXED: Trees placed in your Map Editor generate completely random, unique heights! ---
                if sprite.get('tex') in self.tree_leafy_sprites or sprite.get('tex') == self.tree_dead_sprite:
                    seed_val = int(sprite['x'] * 19 + sprite['y'] * 47)
                    height_scale = 0.75 + (seed_val % 40) / 100.0  # Scales from 75% up to 115% size variations!
        ...
        # NEW: Draw sweeping spark overlays in front of the weapon layer
        for spark in self.sparks:
            pygame.draw.circle(self.screen, spark['color'], (int(spark['x']), int(spark['y'])), spark['size'])

        # --- FIXED LAYER DRAW ORDER: Weapon structures render BEFORE UI panels to sit safely behind menu items ---
        ...
        # --- CORE INTERFACES CHANNEL DRAWN LAST ON TOP ---
        self.draw_weather()
        self.draw_hud()
        self.draw_minimap()
    ...
    def perform_melee_attack(self):
        ...
        # --- FIXED: Melee scanner monitors wall structures for metallic sparks and audio feedback loops ---
        check_dist = 50
        ...
        if 0 <= gx < len(self.map[0]) and 0 <= gy < len(self.map):
            tile_val = self.map[gy][gx]
            # Sparks/clangs trigger on doors, wood, columns (anything except empty, stone, or brick)
            if tile_val not in [0, TileType.WALL_BRICK.value, TileType.WALL_STONE.value]:
                hit_metallic_target = True

        if hit_metallic_target:
            if self.sfx.get("hit_metallic"): self.sfx["hit_metallic"].play()
            # Spawns screen-space sparks where your slice arc connects
